...
La crittografia SSL (Secure Socket Layer) consente di rendere sicure tutte le pagine di destinazione per un’istanza di Marketo.

Quando si compila un modulo Web o si visita una pagina di destinazione ospitata da Marketo, per impostazione predefinita le informazioni vengono inviate attraverso un protocollo non protetto (HTTP). In base alla politica aziendale, potete proteggere le informazioni inviate a Marketo tramite HTTPS. Ad esempio, quando visitate `http://info.mydomain.com/` lo sarà ora `https://info.mydomain.com/`.
