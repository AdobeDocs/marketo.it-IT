---
unique-page-id: 2953338
description: Inviare un'e-mail di vendita Marketo da Microsoft Dynamics - Documenti Marketo - Documentazione del prodotto
title: Inviare un messaggio e-mail di vendita Marketo da Microsoft Dynamics
exl-id: 9ee53a3e-8bbe-458a-9905-a1e1503f9553
feature: Marketo Sales Insights
source-git-commit: a571a312477c6c1b08241ef2cfb011ce525fba07
workflow-type: tm+mt
source-wordcount: '166'
ht-degree: 0%

---

# Inviare un messaggio e-mail di vendita Marketo da Microsoft Dynamics {#send-a-marketo-sales-email-from-microsoft-dynamics}

Il tuo team di vendita può inviare e-mail e farle tracciare per aprire e fare clic direttamente da Microsoft Dynamics. Ecco come.

1. In Dynamics, vai all&#39;area **Vendite**.

   ![](assets/image2014-10-20-11-3a56-3a9.png)

1. Selezionare una **Visualizzazione sistema**.

   ![](assets/image2014-10-20-11-3a56-3a20.png)

1. Seleziona uno o più lead.

   ![](assets/image2014-10-20-11-3a56-3a35.png)

1. Fai clic sulla scheda **...**, quindi fai clic su **Invia e-mail Marketo**.

   ![](assets/image2014-10-20-11-3a56-3a57.png)

   >[!TIP]
   >
   >Puoi [pubblicare le e-mail come modelli](/help/marketo/product-docs/marketo-sales-insight/msi-for-salesforce/features/actions-in-the-msi-panel/send-marketo-email/publish-an-email-to-sales-insight.md) da utilizzare per il team vendite.

1. Aggiungi un oggetto, componi l&#39;e-mail e fai clic su **Invia e-mail**. (Se necessario, scegli un modello).

   ![](assets/image2014-10-20-11-3a57-3a8.png)

   >[!TIP]
   >
   >Puoi fare clic su **Invia e-mail di prova** e l&#39;e-mail ti verrà inviata (indirizzo e-mail principale in Microsoft Dynamics) prima di inviare l&#39;e-mail effettiva.

   >[!NOTE]
   >
   >Puoi inviare fino a 300 e-mail Marketo alla volta.

Ben fatto! Ora sai come inviare e-mail tracciate di Marketo direttamente da Microsoft Dynamics.

>[!MORELIKETHIS]
>
>[Aggiungere un lead/contatto a una campagna Marketo da Microsoft Dynamics](/help/marketo/product-docs/marketo-sales-insight/msi-for-microsoft-dynamics/setting-up-and-using/add-a-lead-contact-to-a-marketo-campaign-from-microsoft-dynamics.md)
